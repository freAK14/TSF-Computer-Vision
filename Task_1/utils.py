...
def draw_boxes(img_names, boxes_dicts, class_names, model_size):
    ...
    for num, img_name, boxes_dict in zip(range(len(img_names)), img_names, boxes_dicts):
        ...
        rgb_img.save("./detection/detection_" + str(num + 1) + ".jpg")
    ...
    for cls in range(len(class_names)):
        ...
        if np.size(boxes) != 0:
            for box in boxes:
                ...
                (text_width, text_height), baseline = cv2.getTextSize(class_names[cls], cv2.FONT_HERSHEY_SIMPLEX, 0.75, 1)               
                cv2.rectangle(frame, (xy[0], xy[1]), (xy[0] + text_width, xy[1] - text_height - baseline), color[::-1], thickness =  cv2.FILLED)             
